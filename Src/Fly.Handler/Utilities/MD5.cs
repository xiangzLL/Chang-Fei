...
namespace Fly.Handler.Utilities
{
    ...
    public sealed class MD5
    {
        ...
        public static string GetHashString(Stream input)
        {
            if (null == input)
            {
                throw new ArgumentNullException(nameof(input), "Unable to calculate hash over null input data");
        ...
        public static byte[] GetHash(Stream input)
        {
            ...
            while (input.Position <= input.Length - 64)
            {
                ...
                MD5.GetHashBlock(data, ref abcd, 0);
            }
            ...
            var hash = MD5.GetHashFinalBlock(finalData, 0, leftDataSize, abcd, input.Length * 8);
